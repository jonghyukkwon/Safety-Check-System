...
with tab1:
    ...
    eval_model = genai.GenerativeModel(
        model_name=MODEL_ID,
        system_instruction=(
          "당신은 감정이 전혀 없는 '엄격한 안전보건 감사 알고리즘'입니다. "
            "다음 3가지 절대 원칙을 따르십시오. "
            "1. 문구 그대로 해석: 계획서에 명시된 텍스트가 없으면 추측하지 말고 '미흡(1점)' 처리하십시오. "
            "2. 보수적 판단: 점수가 3점인지 5점인지 애매한 경우, 무조건 낮은 점수를 부여하십시오. "
            "3. 증거 매핑: 점수를 부여할 때는 반드시 PDF 내의 어떤 문장이나 사진을 보고 판단했는지 '그대로' 인용해야 합니다."
        )
    )
        ...
        if not user_file:
            ...
            with st.spinner("AI가 문서의 이미지와 내용을 정밀 분석 중..."):
                try:
                    ...
                    당신은 수급업체 안전보건관리계획서를 [마스터 가이드라인]에 따라 채점해야 합니다.
                    실행할 때마다 결과가 달라지지 않도록, 아래의 [일관성 유지 규칙]을 반드시 따르세요.
                    
                    [참조: 마스터 가이드라인]
                    {MASTER_GUIDE_TEXT}
                    
                  [🚫 일관성 유지 규칙 (필수 준수)]
                    1. **키워드 매칭**: 가이드라인에 있는 핵심 단어(예: '본사 지원', 'KPI', 'MSDS')가 계획서에 없으면 즉시 감점하십시오.
                    2. **추론 금지**: "작성된 것 같음", "의도는 보임" 등의 주관적 판단을 금지합니다. 명시되지 않았으면 0점 또는 1점입니다.
                    3. **점수 결정 트리**:
                       - 증빙(사진/서류)과 내용이 모두 완벽함 -> 우수 (5/10점)
                       - 내용은 있으나 구체성이 부족하거나 사진이 누락됨 -> 보통 (3/5점)
                       - 내용이 없거나 다른 공사 내용 복사/붙여넣기 -> 미흡 (1점)
                       - **중요:** '우수'와 '보통' 사이에서 고민되면 무조건 '보통'을 선택하십시오. (보수적 접근)
                    4. **공종 일치성**: PDF 상단의 공사명/내용과 본문의 안전관리 내용이 서로 다르면(예: 용접공사인데 전기안전 내용만 있음) 해당 항목은 0점입니다.
                    5. **중대재해(17번)**: 이력 '없음'이 명시되어야 0점입니다. 언급이 없으면 확인 불가로 간주하여 0점 처리하되 코멘트를 남기세요.
                    ...
                    [
                        {{
                            ...
                            "score": 1,
                            "max_score": 5,
                            "evidence": "조직도는 있으나, 본사 안전팀의 정기 지원 계획에 대한 구체적 문구가 발견되지 않음.",
                            "judgment": "미흡"
                        }},
                        ... (17번까지 반복)
                    ]
